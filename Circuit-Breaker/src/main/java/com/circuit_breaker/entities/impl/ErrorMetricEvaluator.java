...
public class ErrorMetricEvaluator extends MetricEvaluator implements Runnable {
    private State state;
    private volatile boolean isRunning = false;

    public ErrorMetricEvaluator(State state, MetricEvaluatorConfig metricEvaluatorConfig, List<Metric<?>> metrics) {
    ...
    public boolean evaluate() {
        ...
        for (int index = metrics.size() - 1; index >= 0; index--) {
            ...
            if (Objects.equals(metrics.get(index).getMetricValue(), metricEvaluatorConfig.getResponseType()))
                failures++;
            total++;
        }
